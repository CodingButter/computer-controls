...
describe("the shell", () => {
  test("draws and does nothing else", () => {
    ...
    expect(resident.focusable).toBe(true);
  ...
  test("being demonstrable changes the window's manners and nothing else", () => {
    // Demo mode existed because an unfocusable window is override-redirect on
    // X11 and therefore absent from every list of windows — unrecordable by
    // OBS, by a screen recorder, and by this project's own window capture. It
    // bought that back with three options, of which `focusable` was the whole
    // point.
    //
    // That flag is now true in both modes, because the same X11 fact was also
    // silently discarding `alwaysOnTop` and leaving the orb's visibility to
    // stacking luck — a bug for every user, not just the recording ones. So the
    // difference is down to two, and this is the assertion that keeps it two:
    // any future option that leaks into the demo branch fails here, including
    // one that loosened something that matters.
    const resident = windowOptionsFor({ demo: false });
    ...
    expect(differing.sort()).toEqual(["skipTaskbar", "title"]);

    // Recordability is no longer demo mode's to grant: both windows are
    // managed by the window manager, so both are in every list of windows.
    expect(resident.focusable).toBe(true);
    expect(demo.focusable).toBe(true);

    // What the mode still buys: a place in the switcher, and a name a person
