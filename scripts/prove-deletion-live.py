...
def render(deletion: dict, contrast: dict | None, env: dict[str, str], element_id: str) -> str:
    checks = acceptance(deletion)
...
def main() -> int:
    ...
    if deletion is None:
        print(
            "no deletion was reported before the timeout. If text was deleted and this "
            "still says nothing, that is the finding — do not re-run until it passes.",
            file=sys.stderr,
        )
        return 1
