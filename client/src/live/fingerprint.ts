/**
 * The wake phrase is a shape in the audio, not a word in a transcript.
 *
 * Transcription wake died three times in the field: a 26MB model that crashes
 * on short inputs, a language model that hears "Hej Mastra!" in Swedish, and a
 * closed gate every time the transcript came back empty. This module replaces
 * all of it with a shape comparison: the utterance's MFCC sequence against a
 * bank of "hey mastra" templates, matched by subsequence DTW so the phrase is
 * found ANYWHERE in the utterance — the acoustic version of the 07530c5
 * lesson ("the wake is the phrase, heard anywhere").
 *
 * Everything here is pure and browser-safe: no Node globals, no imports the
 * emitted file would carry (the type import below erases), no state beyond
 * what the caller hands in. It runs on the renderer thread next to the VAD,
 * so the cost budget is real — see the timing test.
 */
import type { AudioFrame, WakeWordDetector } from "./ear.ts";

/**
 * One reference rendering of the wake phrase: an id for provenance, the MFCC
 * frames as plain arrays (JSON-portable — the factory bank ships as committed
 * JSON), and an optional weight. Factory templates weigh 1; enrolled templates
 * carry ENROLLED_WAKE_WEIGHT, which divides their distance and so lowers the
 * bar for the one voice that actually lives with this machine.
 */
 ...
 */
export function mfcc(samples: Int16Array, sampleRate: number): Float32Array[] {
  ...
  for (let f = 0; f < frameCount; f++) {
    ...
    for (let m = 0; m < MEL_FILTERS; m++) {
      ...
      for (let k = left; k < center; k++) {
        if (center > left) sum += power[k] * ((k - left) / (center - left));
      }
      for (let k = center; k < right; k++) {
        if (right > center) sum += power[k] * ((right - k) / (right - center));
      }
  ...
  for (const frame of frames) {
    for (let d = 0; d < CEPSTRAL_COEFFICIENTS; d++) mean[d] += frame[d];
  }
  for (let d = 0; d < CEPSTRAL_COEFFICIENTS; d++) mean[d] /= frames.length;
  for (const frame of frames) {
    for (let d = 0; d < CEPSTRAL_COEFFICIENTS; d++) frame[d] -= mean[d];
  }

  return frames;
}

function frameDistance(a: ArrayLike<number>, b: ArrayLike<number>): number {
  let sum = 0;
 ...
 */
export function subsequenceDtw(query: Float32Array[] | number[][], template: Float32Array[] | number[][]): number {
  ...
  for (let j = 0; j < n; j++) {
    ...
    if (span < m * (1 - BAND_RATIO) - slack || span > m * (1 + BAND_RATIO) + slack) continue;
 ...
 */
export function createFingerprintDetector(
