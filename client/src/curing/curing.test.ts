...
function install(dir: string, id: string, exec: string, extra = ""): DesktopEntryApp {
  ...
  return { name: id.replace(".desktop", ""), desktopId: id, exec, sourcePath: file };
}
...
describe("curing one Exec line", () => {
  ...
  it("leaves a non-Chromium file alone entirely", () => {
    ...
    expect(cureDesktopFile(source)).toEqual({ text: source, changed: false });
  });
});
...
describe("curing the machine", () => {
  it("writes a user-scope override and never touches the system file", () => {
    ...
    const discord = install(systemDir, "discord.desktop", "/usr/share/discord/Discord");
    const before = fs.readFileSync(discord.sourcePath as string, "utf8");

    const report = cureChromiumApps({
      rows: [row({ name: "Discord", desktopId: "discord.desktop", running: true })],
      entries: [discord],
      userApplicationsDir: userDir,
    });

    expect(report.cured).toEqual([{ name: "Discord", desktopId: "discord.desktop" }]);
    expect(fs.readFileSync(discord.sourcePath as string, "utf8")).toBe(before);
  ...
  it("never cures an application the user has not permitted", () => {
    const { systemDir, userDir } = machine();
    const chrome = install(systemDir, "chrome.desktop", "/usr/bin/google-chrome-stable %U");

    const report = cureChromiumApps({
      rows: [row({ name: "chrome", desktopId: "chrome.desktop", permitted: false })],
      entries: [chrome],
      userApplicationsDir: userDir,
    });
  ...
  it("is idempotent across runs — the second cure writes nothing", () => {
    const { systemDir, userDir } = machine();
    const slack = install(systemDir, "slack.desktop", "/usr/bin/slack -s %U");
    const rows = [row({ name: "Slack", desktopId: "slack.desktop" })];
    ...
    const second = cureChromiumApps({
      ...
      entries: [{ ...slack, exec: `/usr/bin/slack ${ACCESSIBILITY_FLAG} -s %U`, sourcePath: target }],
      userApplicationsDir: userDir,
    });

    expect(second.cured).toEqual([]);
    expect(second.alreadyCured).toEqual([{ name: "Slack", desktopId: "slack.desktop" }]);
  ...
  it("leaves permitted non-Chromium applications out of the report", () => {
    const { systemDir, userDir } = machine();
    const files = install(systemDir, "org.gnome.Nautilus.desktop", "nautilus --new-window %U");

    const report = cureChromiumApps({
      rows: [row({ name: "Files", desktopId: "org.gnome.Nautilus.desktop", running: true })],
      entries: [files],
      userApplicationsDir: userDir,
    });
  ...
  it("names an already-cured application that is running unreadable — it predates its cure", () => {
    ...
    const report = cureChromiumApps({
      rows: [
        row({ name: "Discord", desktopId: "discord.desktop", running: true, readable: false }),
      ],
      entries: [discord],
      userApplicationsDir: userDir,
    });
